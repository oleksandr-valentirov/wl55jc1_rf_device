...
/* The library's deck on this silicon, against the published one. ADR-0030
 * radio_devices_docs/wl55_device/radio/hopping.md */
static int8_t hop_deck_kat(hop_prf_t prf) {
    hop_ctx_t kat;
    uint8_t ch;

    if (hop_init(&kat, prf, (void *)HV_HOP_KEY, HOP_VEC_COUNT) != 0)
        return -1;
...
void vectors_report(vectors_report_t *out) {
    ...
    out->hop_deck_rc = hop_deck_kat(hop_prf_aes);

    /* The negative case: a KAT never red reads in neither direction.
     * radio_devices_docs/radio/hopping.md */
    out->hop_deck_ctl_rc = hop_deck_kat(hop_prf_swap32);
}
