...
# wire_v3's published hop key, so this set ties to that one.
...
# First cycle, cycle boundary, a far jump landed on directly, a wrap-adjacent value.
# radio_devices_docs/wl55_device/testing/host-tests.md
...
def check_properties():
    ...
    for sf in range(HOP_COUNT * cycles):
        ...
        seen[ch] += 1
        if prev == ch:
            # Measured, not asserted: repeats across a boundary are 1/count by design.
            # radio_devices_docs/wl55_device/testing/host-tests.md
...
# The digest covers the values, not the file: text hashing moved it on a reword.
# radio_devices_docs/wl55_device/testing/host-tests.md
EMITTED = []
...
# A non-zero counter, so a byte-order error cannot hide in a block of zeroes.
# radio_devices_docs/wl55_device/testing/host-tests.md
PRF_IN = bytearray(16)
...
def main():
    # A regression guard, not an anchor: the two CRYP peripherals close the circle.
    # radio_devices_docs/wl55_device/testing/host-tests.md
    ...
    # Cycle 0 reads the same under both conventions, so it passes for 56 seconds.
    # radio_devices_docs/wl55_device/testing/host-tests.md
    ...
    lines = [
        ...
        "",
        # FIPS-197 C.1: the only block here a shared mistake could not have produced.
        # radio_devices_docs/wl55_device/testing/host-tests.md
